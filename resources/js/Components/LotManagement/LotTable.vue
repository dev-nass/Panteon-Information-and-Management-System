...
const openDeleteLotModal = (lot) => {
    lotToDelete.value = lot;
    nextTick(() => {
        const el = document.getElementById("delete-lot-modal");
        try {
            if (el && window.HSOverlay) {
                window.HSOverlay.open(el);
            } else if (typeof HSOverlay !== "undefined") {
                HSOverlay.open("#delete-lot-modal");
            }
        } catch {
            if (typeof HSOverlay !== "undefined") HSOverlay.open("#delete-lot-modal");
        }
    });
};
...
const confirmDeleteLot = () => {
    ...
    const lotLabel = `${lotToDelete.value.column}${lotToDelete.value.row}`;
    router.delete(route("admin.lot_management.delete.lot", lotToDelete.value.id), {
        onSuccess: () => {
            HSOverlay.close("#delete-lot-modal");
            lotToDelete.value = null;
            fetchLots();
            toast.success(`Lot "${lotLabel}" deleted successfully!`, {
                duration: 3000,
            });
        },
        onError: () => {
...
<template>
                ...
                >
                    ...
                    <TableData>
                        ...
                        >
                            ...
                            <button
                                @click.stop="openDeleteLotModal(lot)"
    ...
    <Teleport to="body">
        <div
            ...
            role="dialog"
            tabindex="-1"
            aria-labelledby="delete-lot-modal-label"
        >
            <div
                    ...
                    <div class="p-10 flex flex-col items-center gap-y-4 text-center">
                        ...
                        <p class="text-gray-600 dark:text-neutral-300 max-w-sm">
                            ...
                            <span class="font-semibold text-gray-900 dark:text-white">
                                LOT {{ lotToDelete ? `${lotToDelete.column}${lotToDelete.row}` : "" }}
                            </span>
